...
function loadTimingDataFromFile() {
    try {
        // Look for timing data in common locations
        const possiblePaths = [
            'target/timing.json',
            'timing.json',
        ...
        for (const filePath of possiblePaths) {
            if (fs.existsSync(filePath)) {
                console.log(`Loading timing data from ${filePath}...`);
                const content = fs.readFileSync(filePath, 'utf8');
                
                // Parse Criterion benchmark output format
                const timingData = parseCriterionOutput(content);
                
                if (timingData.length > 0) {
                    console.log(`Parsed ${timingData.length} benchmark results`);
                    return timingData;
                }
            }
        }
        
        console.error("No timing data file found. Please run benchmarks first.");
        throw new Error("No timing data file found. Please run benchmarks first.");
    } catch (error) {
...
function generateCachedAccessChart(results) {
    ...
    let chart = '```mermaid\nxychart-beta\n';
    chart += '    title "Best Cached Access Performance"\n';
    chart += '    x-axis "Implementation" [';
